...
describe('Route /api/dashboard/repositories/[name]', () => {
  ...
  it('returns a single repository from the dashboard service', async () => {
    ...
    const response = await GET(new Request('http://localhost/api/dashboard/repositories/7'), {
      params: Promise.resolve({ name: '7' }),
    });
    ...
    expect(getRepositoryMock).toHaveBeenCalledWith(7);
  });

  it('returns 400 for invalid repository path ids', async () => {
  ...
  it('maps repository lookup failures to integration error responses', async () => {
