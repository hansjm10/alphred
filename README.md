...
- The e2e webserver isolates state per suite under `apps/dashboard/.e2e-runtime/port-<port>/` and resets it on each run, so test runs do not pollute the default `alphred.db` or normal sandbox/worktree paths.
