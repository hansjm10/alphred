# AGENTS.md - Alphred Project Guide
...
- Tests should run in clean checkouts without prebuilt `packages/*/dist`.
- Keep tests co-located with source files (`*.test.ts`).
- Keep Next.js App Router tests co-located in `app/` (`*.test.tsx`).
...
- Prefer package-local guidance when present (for example `packages/*/AGENTS.md`).
- Keep root guidance broad; keep package-specific rules close to implementation.
