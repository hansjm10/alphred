...
export function migrateDatabase(db: AlphredDatabase): void {
  ...
  tx.run(sql`CREATE TABLE IF NOT EXISTS work_item_workflow_runs (
    ...
    CONSTRAINT work_item_workflow_runs_repository_id_work_item_id_fk
      FOREIGN KEY (repository_id, work_item_id)
      REFERENCES work_items(repository_id, id)
      ON DELETE CASCADE
  )`);
  tx.run(sql`CREATE UNIQUE INDEX IF NOT EXISTS work_item_workflow_runs_work_item_id_workflow_run_id_uq
    ON work_item_workflow_runs(work_item_id, workflow_run_id)`);
  tx.run(sql`CREATE INDEX IF NOT EXISTS work_item_workflow_runs_repository_id_work_item_linked_at_idx
    ON work_item_workflow_runs(repository_id, work_item_id, linked_at)`);
  tx.run(sql`CREATE INDEX IF NOT EXISTS work_item_workflow_runs_workflow_run_id_idx
    ON work_item_workflow_runs(workflow_run_id)`);
  tx.run(sql`CREATE INDEX IF NOT EXISTS work_item_workflow_runs_linked_at_idx
    ON work_item_workflow_runs(linked_at)`);
  ...
  tx.run(sql`CREATE INDEX IF NOT EXISTS story_workspaces_repository_id_status_idx
    ON story_workspaces(repository_id, status)`);
  tx.run(sql`CREATE INDEX IF NOT EXISTS story_workspaces_created_at_idx
    ON story_workspaces(created_at)`);
  tx.run(sql`CREATE TRIGGER IF NOT EXISTS story_workspaces_story_type_insert_ck
    BEFORE INSERT ON story_workspaces
    FOR EACH ROW
    WHEN (
      SELECT type
      FROM work_items
      WHERE id = NEW.story_work_item_id
    ) <> 'story'
    BEGIN
      SELECT RAISE(ABORT, 'story_workspaces.story_work_item_id must reference a story work_item');
    END`);
  tx.run(sql`CREATE TRIGGER IF NOT EXISTS story_workspaces_story_type_update_ck
    BEFORE UPDATE OF story_work_item_id ON story_workspaces
    FOR EACH ROW
    WHEN (
      SELECT type
      FROM work_items
    ...
    BEGIN
      SELECT RAISE(ABORT, 'story_workspaces.story_work_item_id must reference a story work_item');
    END`);
  tx.run(sql`CREATE TRIGGER IF NOT EXISTS work_items_story_workspace_type_update_ck
    BEFORE UPDATE OF type ON work_items
    FOR EACH ROW
    WHEN (
      OLD.type = 'story'
      AND NEW.type <> 'story'
      AND EXISTS (
        ...
        WHERE story_work_item_id = NEW.id
      )
    )
    BEGIN
      SELECT RAISE(ABORT, 'work_items.type must remain story while referenced by story_workspaces');
    END`);
