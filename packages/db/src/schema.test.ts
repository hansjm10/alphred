...
describe('database schema hardening', () => {
  ...
  it('backfills run_nodes execution columns from source nodes during legacy upgrades', () => {
    ...
    db.run(sql`INSERT INTO workflow_trees (id, tree_key, version, name)
      VALUES (1, 'legacy_run_nodes_tree', 1, 'Legacy run_nodes tree')`);
    db.run(sql`INSERT INTO prompt_templates (id, template_key, version, content, content_type)
      VALUES (1, 'legacy_run_nodes_prompt', 1, 'Legacy prompt', 'text')`);
  ...
  it('enforces story workspace ownership and one-workspace-per-story invariants', () => {
    const db = createDatabase(':memory:');
    migrateDatabase(db);
    ...
    expect(() =>
      db.insert(storyWorkspaces).values({
        ...
        worktreePath: '/tmp/alphred/worktrees/story-workspace-feature',
        branch: 'alphred/story/feature',
        baseBranch: 'main',
      }).run(),
    ).toThrow('story_workspaces.story_work_item_id must reference a story work_item');
    ...
    expect(() =>
      db.insert(storyWorkspaces).values({
        ...
        status: 'active',
        statusReason: null,
        lastReconciledAt: '2026-03-05T10:01:00.000Z',
        removedAt: null,
      }).run(),
    ).toThrow();
    ...
    expect(() =>
      db
        ...
        .run(),
    ).toThrow('work_items.type must remain story while referenced by story_workspaces');
  });

  it('enforces story workspace lifecycle status and removal timestamp invariants', () => {
    const db = createDatabase(':memory:');
    migrateDatabase(db);
    ...
    const repository = db
      .insert(repositories)
      .values({
        name: 'story-workspace-lifecycle-constraints',
        provider: 'github',
        remoteUrl: 'https://github.com/acme/story-workspace-lifecycle-constraints.git',
        remoteRef: 'acme/story-workspace-lifecycle-constraints',
        defaultBranch: 'main',
        cloneStatus: 'cloned',
      })
      .returning({ id: repositories.id })
    ...
    const story = db
      ...
      .values({
        ...
        type: 'story',
        status: 'Draft',
        title: 'Story workspace lifecycle checks',
        revision: 0,
      })
      .returning({ id: workItems.id })
      .get();
    ...
    expect(() =>
      db.insert(storyWorkspaces).values({
        repositoryId: repository.id,
        storyWorkItemId: story.id,
        worktreePath: '/tmp/alphred/worktrees/story-workspace-invalid-status',
        branch: 'alphred/story/invalid-status',
        baseBranch: 'main',
        status: 'unknown',
        statusReason: null,
    ...
    expect(() =>
      db.insert(storyWorkspaces).values({
        repositoryId: repository.id,
        storyWorkItemId: story.id,
        worktreePath: '/tmp/alphred/worktrees/story-workspace-stale-has-removed-at',
        branch: 'alphred/story/stale-has-removed-at',
        baseBranch: 'main',
        status: 'stale',
        statusReason: 'missing_path',
        lastReconciledAt: '2026-03-05T10:00:00.000Z',
        removedAt: '2026-03-05T10:00:00.000Z',
      }).run(),
    ).toThrow();
    ...
    expect(() =>
      db.insert(storyWorkspaces).values({
        repositoryId: repository.id,
        storyWorkItemId: story.id,
        worktreePath: '/tmp/alphred/worktrees/story-workspace-removed-missing-removed-at',
        branch: 'alphred/story/removed-missing-removed-at',
        baseBranch: 'main',
        status: 'removed',
        statusReason: 'cleanup_requested',
        lastReconciledAt: '2026-03-05T10:00:00.000Z',
    ...
    expect(() =>
      db.insert(storyWorkspaces).values({
        repositoryId: repository.id,
        storyWorkItemId: story.id,
        worktreePath: '/tmp/alphred/worktrees/story-workspace-empty-reason',
        branch: 'alphred/story/empty-reason',
        baseBranch: 'main',
        status: 'stale',
        statusReason: '',
        lastReconciledAt: '2026-03-05T10:00:00.000Z',
